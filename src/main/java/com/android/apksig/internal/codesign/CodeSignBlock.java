...
public final class CodeSignBlock {
    ...
    public static byte[] build(
        ...
        int hapInfoSize = buildHapInfoSegment(rootHash, signature, dataSize, mapOffset, mapSize).length;
        int nativeLibInfoSize = NATIVE_LIB_INFO_STUB_SIZE;
        ...
        // HapInfoSegment
        buf.put(buildHapInfoSegment(rootHash, signature, dataSize, mapOffset, mapSize));

        // NativeLibInfoSegment stub (12 bytes)
        buf.putInt(NATIVE_LIB_INFO_MAGIC);
        buf.putInt(NATIVE_LIB_INFO_STUB_SIZE);
        buf.putInt(0); // sectionNum = 0
